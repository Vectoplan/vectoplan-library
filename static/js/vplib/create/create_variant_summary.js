/* services/vectoplan-library/static/js/vplib/create/create_variant_summary.js */
(function () {
  "use strict";
  ...
  function getVariable(fieldKey) {
    try {
      ...
      return maps.variablesByKey[fieldKey] || maps.variablesById[fieldKey] || null;
    } catch (error) {
      return null;
    }
  }
  ...
  function getUnit(unitId) {
    try {
      ...
      return maps.unitsById[unitId] || maps.unitsBySymbol && maps.unitsBySymbol[unitId] || null;
    } catch (error) {
      return null;
    }
  }
  ...
  function optionLabel(variable, value) {
    try {
      ...
      var options = U().toArray(variable.options || variable.enum || variable.enum_values || variable.enumValues || variable.allowed_values || variable.allowedValues);
      ...
      if (variable.key === "material.type") {
        ...
        if (material) {
          return material.label || material.name || material.id || value;
        }
      }
  ...
  function formatValue(value, variable, options) {
    try {
      ...
      if (type === "number" || type === "integer" || type === "int" || type === "money" || type === "float" || type === "decimal" || widget === "number" || widget === "money") {
      ...
      if (value && typeof value === "object") {
        return truncateText(U().safeJsonStringify(value, "{}", 0), config.maxPartLength || runtime.options.maxPartLength);
      }
  ...
  function getCardConfig(profile) {
    try {
      ...
      return {
        title_field: cards.title_field || cards.titleField || "variant.label",
        subtitle_fields: U().toArray(cards.subtitle_fields || cards.subtitleFields),
        badge_fields: U().toArray(cards.badge_fields || cards.badgeFields)
      };
    } catch (error) {
      return {
  ...
  function getSummaryFields(profile) {
    try {
      var fields = U().toArray(profile && (profile.summary_fields || profile.summaryFields));
  ...
  function buildSummaryPayload(values, profile, options) {
    try {
      ...
      return {
        ...
        profile_id: sourceProfile ? sourceProfile.id || sourceProfile.profile_id || "" : "",
        values: sourceValues,
  ...
  function updateVariantSummaryPayload(variant, profile) {
    try {
      ...
      source.definitionSummary = payload.text;
      source.definition_values = values;
      source.definitionValues = values;
      source.definition_values_json = U().valuesToJson(values);
      source.definitionValuesJson = source.definition_values_json;
    ...
    try {
      ...
      document.addEventListener("vectoplan:create:variant-values-changed", function (event) {
        try {
          ...
          if (detail.__vp_variant_summary_event) {
            return;
          }
      ...
      document.addEventListener("vectoplan:create:variant-drawer-opened", function (event) {
        try {
          ...
          } else if (payload.definitionValues && typeof payload.definitionValues === "object") {
            values = payload.definitionValues;
          } else if (payload.values && typeof payload.values === "object") {
            values = payload.values;
          } else if (payload.definition_values_json || payload.definitionValuesJson || payload.valuesJson || payload.values_json) {
            values = U().valuesFromJson(payload.definition_values_json || payload.definitionValuesJson || payload.valuesJson || payload.values_json);
          }
      ...
      document.addEventListener("vectoplan:create:variant-table-rendered", function () {
        try {
          scheduleTableSummaries(null, {
            source: "table_rendered",
            force: true
          });
        } catch (error) {
          warn("Table rendered summary listener failed.", error);
        }
      });
