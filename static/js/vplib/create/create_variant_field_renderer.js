// services/vectoplan-library/static/js/vplib/create/create_variant_field_renderer.js
(function () {
  "use strict";
  ...
  var fallbackUtils = {
    ...
    getProfileFieldKeys: function (profile) {
      try {
        ...
        fallbackUtils.toArray(profile && profile.sections).forEach(function (section) {
          fallbackUtils.toArray(section && section.fields).forEach(function (field) {
            var fieldKey = typeof field === "string"
              ? field
              : field && (field.key || field.field_key || field.fieldKey || field.variable_key || field.variableKey || field.id);
        ...
        fallbackUtils.toArray(profile && (profile.all_fields || profile.allFields)).forEach(function (fieldKey) {
          fieldKey = fallbackUtils.normalizeFieldKey(fieldKey);
        ...
        fallbackUtils.toArray(profile && (profile.required_fields || profile.requiredFields)).forEach(function (fieldKey) {
          fieldKey = fallbackUtils.normalizeFieldKey(fieldKey);
        ...
        fallbackUtils.toArray(profile && (profile.optional_fields || profile.optionalFields)).forEach(function (fieldKey) {
          fieldKey = fallbackUtils.normalizeFieldKey(fieldKey);
    ...
    isFieldRequired: function (profile, fieldKey) {
      try {
        return fallbackUtils.toArray(profile && (profile.required_fields || profile.requiredFields)).indexOf(fieldKey) !== -1;
      } catch (error) {
        return false;
      }
    },
    ...
    shouldHideVariableInDrawer: function (variable) {
      try {
        ...
        var key = variable.key || variable.variable_key || variable.variableKey || variable.id || "";
        ...
        return fallbackUtils.bool(metadata.hide_in_create_drawer, false) ||
          fallbackUtils.bool(metadata.hideInCreateDrawer, false) ||
          fallbackUtils.bool(metadata.system_managed, false) ||
          fallbackUtils.bool(metadata.systemManaged, false) ||
    ...
    normalizeValueForVariable: function (value, variable) {
      try {
        var type = variable && (variable.value_type || variable.valueType || variable.data_type || variable.dataType || variable.type)
          ? String(variable.value_type || variable.valueType || variable.data_type || variable.dataType || variable.type)
          : "";
        ...
        if (type === "document_list" || type === "documentList" || type === "array" || type === "multi_enum" || type === "multi-enum") {
          if (Array.isArray(value)) {
            return value;
          }
    ...
    defaultValueForVariable: function (variable) {
      try {
        ...
        var type = variable.value_type || variable.valueType || variable.data_type || variable.dataType || variable.type || "string";
        ...
        if (type === "document_list" || type === "array" || type === "multi_enum" || type === "multi-enum") {
          return [];
        }
  ...
  var runtime = {
    ...
    fieldEventsBound: typeof WeakSet !== "undefined" ? new WeakSet() : null,
    fallbackFieldEventsBound: {},
  ...
  function readDrawerContext(cache) {
    try {
      ...
      return {
        ...
        taxonomy_path: [
          U().attr(c.drawer, "data-vp-current-domain", ""),
          U().attr(c.drawer, "data-vp-current-category", ""),
          U().attr(c.drawer, "data-vp-current-subcategory", "")
        ].filter(Boolean).join("/"),
  ...
  function getWidget(variable) {
    try {
      ...
      if (type === "document_list" || type === "documents" || type === "file_list") {
  ...
  function syncDocumentList(container) {
    try {
      ...
      if (hidden) {
        setFieldValueSilently(hidden, U().safeJsonStringify(list, "[]"));
      }
      ...
      container.addEventListener("click", function (event) {
        try {
          ...
          if (target.closest("[data-vp-document-add='true']")) {
            ...
            dispatchValuesChanged(container, fieldKey, syncDocumentList(container), collectValues(getDrawer(container)), variable);
            return;
          }
  ...
  function renderControl(fieldKey, variable, value, required, context) {
    try {
      ...
      if (widget === "document_list" || type === "document_list" || type === "documents" || type === "file_list") {
  ...
  function normalizeRenderableSections(profile) {
    try {
      ...
      fallbackKeys = fallbackKeys.concat(U().toArray(profile && (profile.required_fields || profile.requiredFields)));
      fallbackKeys = fallbackKeys.concat(U().toArray(profile && (profile.optional_fields || profile.optionalFields)));
  ...
  function buildRenderSignature(profile, values, context) {
    try {
  ...
  function renderProfile(input, root) {
    try {
      ...
      var sections = normalizeRenderableSections(profile);
      var signature = buildRenderSignature(profile, values, context);

      if (runtime.renderInProgress && signature === runtime.lastRenderSignature) {
        runtime.suppressedRenderCount += 1;
        return {
          ok: true,
          skipped: true,
          reason: "duplicate_render",
          profile: profile,
          values: values,
          context: context
        };
        ...
        window.VectoplanCreateVariantProfiles
          ...
          .then(function (emptyResult) {
            renderProfile({
              ...
              values: emptyResult.values || emptyResult.empty_values || {},
        ...
        return window.VectoplanCreateVariantProfiles
          ...
          .then(function (bundle) {
            renderProfile({
              ...
              values: U().safeMerge(bundle.empty_values || bundle.emptyValues || {}, config.values || {}),
  ...
  function isFieldEventsBound(drawer) {
    try {
      if (!drawer) {
        return true;
      }

      if (runtime.fieldEventsBound && runtime.fieldEventsBound.has(drawer)) {
        return true;
      }

      var id = drawer.id || U().attr(drawer, "data-vp-drawer-id", "");

      if (id && runtime.fallbackFieldEventsBound[id]) {
        return true;
      }

      return drawer.getAttribute("data-vp-variant-field-events-bound") === "true";
    } catch (error) {
      return false;
    }
  }

  function markFieldEventsBound(drawer) {
    try {
      if (!drawer) {
        return false;
      }

      if (runtime.fieldEventsBound) {
        runtime.fieldEventsBound.add(drawer);
      }

      var id = drawer.id || U().attr(drawer, "data-vp-drawer-id", "");

      if (id) {
        runtime.fallbackFieldEventsBound[id] = true;
      }
  ...
  function bindFieldEvents(root) {
    try {
      ...
      if (!drawer || isFieldEventsBound(drawer)) {
        return;
      }
    ...
    try {
      ...
      document.addEventListener("vectoplan:create:variant-empty-values-ready", function (event) {
        try {
          ...
          renderProfile({
            profile: profile,
            values: detail.values || detail.empty_values || {},
  ...
  function getRuntimeSnapshot() {
    try {
      ...
      return {
        ...
        renderInProgress: runtime.renderInProgress,
