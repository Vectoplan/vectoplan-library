/* services/vectoplan-library/static/js/vplib/create/create_payload.js */
(function () {
  "use strict";
  ...
  function collectPayload(form, options) {
    try {
      ...
      augmentPayloadWithContext(payload, safeForm);
  ...
  function augmentPayloadWithContext(payload, form) {
    try {
      if (!payload) {
        return payload;
      }
      ...
      payload.domain = payload.domain || context.domain;
      payload.category = payload.category || context.category;
      payload.subcategory = payload.subcategory || context.subcategory;
      payload.taxonomy_path = payload.taxonomy_path || context.taxonomy_path || buildTaxonomyPath(context);
      payload.taxonomyPath = payload.taxonomyPath || payload.taxonomy_path;
      payload.object_kind = payload.object_kind || context.object_kind;
      payload.objectKind = payload.objectKind || payload.object_kind;

      payload.family_profile_id = payload.family_profile_id || context.family_profile_id || "";
      payload.familyProfileId = payload.familyProfileId || payload.family_profile_id || "";
      payload.variant_profile_id = payload.variant_profile_id || context.variant_profile_id || "";
      payload.variantProfileId = payload.variantProfileId || payload.variant_profile_id || "";

      var uid = getExistingVplibUid(payload, safeForm, createContext, generatorContext);
      if (uid) {
        payload.vplib_uid = uid;
        payload.vplibUid = uid;
      }

      payload.generator_context_uid = payload.generator_context_uid ||
        payload.generatorContextUid ||
        getNested(generatorContext, ["context_uid"], "") ||
        getNested(generatorContext, ["data", "context_uid"], "");

      payload.generatorContextUid = payload.generatorContextUid || payload.generator_context_uid;

      payload.generator_context_schema_version = payload.generator_context_schema_version ||
        payload.generatorContextSchemaVersion ||
        getNested(generatorContext, ["schema_version"], "") ||
        getNested(generatorContext, ["data", "schema_version"], "");

      payload.generatorContextSchemaVersion = payload.generatorContextSchemaVersion || payload.generator_context_schema_version;

      return payload;
    } catch (error) {
      safeWarn("Augment payload with context failed.", error);
      return payload;
    }
  }
  ...
  function normalizePayloadBeforeSend(payload, form) {
    try {
      ...
      var objectKind = String(
        firstScalar(payload.object_kind) ||
        firstScalar(payload.objectKind) ||
      ...
      payload.family_name = firstScalar(payload.family_name || payload.familyName || "").trim();
      payload.familyName = payload.familyName || payload.family_name;
      payload.family_description = firstScalar(payload.family_description || payload.familyDescription || "").trim();
      payload.familyDescription = payload.familyDescription || payload.family_description;
      payload.material_class = firstScalar(payload.material_class || payload.materialClass || "").trim();
      ...
      payload.taxonomy_path = normalizeTaxonomyPath(payload.taxonomy_path || payload.taxonomyPath || buildTaxonomyPath(payload));
      payload.taxonomyPath = payload.taxonomy_path;

      payload.geometry_width = String(firstScalar(payload.geometry_width || payload.geometryWidth || "1.00")).trim();
      payload.geometryWidth = payload.geometry_width;
      payload.geometry_height = String(firstScalar(payload.geometry_height || payload.geometryHeight || "1.00")).trim();
      payload.geometryHeight = payload.geometry_height;
      payload.geometry_depth = String(firstScalar(payload.geometry_depth || payload.geometryDepth || "1.00")).trim();
      payload.geometryDepth = payload.geometry_depth;
      payload.geometry_unit = String(firstScalar(payload.geometry_unit || payload.geometryUnit || "m")).trim();
      payload.geometryUnit = payload.geometry_unit;
      payload.primitive_shape = normalizeToken(payload.primitive_shape || payload.primitiveShape || "block", "block");
      payload.primitiveShape = payload.primitive_shape;

      normalizeSingleValueFields(payload);
  ...
  function applyPayloadContract(payload, form, options) {
    try {
      ...
      var requiredFields = getContractRequiredFields(contract);
      var duplicateGuards = getContractDuplicateGuards(contract);
      var issues = [];

      duplicateGuards.forEach(function (fieldName) {
        if (payload[fieldName] !== undefined) {
          payload[fieldName] = firstScalar(payload[fieldName]);
        }
      });

      SCALAR_FIELD_NAMES.forEach(function (fieldName) {
        if (payload[fieldName] !== undefined) {
          payload[fieldName] = firstScalar(payload[fieldName]);
        }
      });

      JSON_FIELD_NAMES.forEach(function (fieldName) {
        if (payload[fieldName] === undefined || payload[fieldName] === null) {
          return;
        }

        if (typeof payload[fieldName] !== "string") {
          payload[fieldName] = stringifyJson(payload[fieldName]);
        }
      });
      ...
      requiredFields.forEach(function (fieldName) {
        if (payload[fieldName] === undefined || payload[fieldName] === null || String(payload[fieldName]).trim() === "") {
          issues.push({
            ...
            field: fieldName,
            message: "Required create payload field is missing: " + fieldName
          });
        }
      });

      payload.payload_contract_schema_version = contract.schema_version || contract.schemaVersion || "create_payload.v1";
      payload.payloadContractSchemaVersion = payload.payload_contract_schema_version;
      payload.client_payload_version = PAYLOAD_VERSION;
      payload.clientPayloadVersion = PAYLOAD_VERSION;

      if (issues.length) {
        payload.client_validation_issues = issues;
        payload.clientValidationIssues = issues;
      }

      localState.lastValidation = {
        ok: issues.length === 0,
        issues: clone(issues),
        checkedAt: timestamp()
      };
      ...
      safeWarn("Apply payload contract failed.", error);
      return payload;
    }
  }

  function getContractRequiredFields(contract) {
    try {
      var source = contract || {};
      var fields = source.required_fields ||
        source.requiredFields ||
        getNested(source, ["stable_fields", "identity", "required"], null) ||
        [];

      if (!Array.isArray(fields)) {
        return ["family_name", "domain", "category", "subcategory", "object_kind"];
      }

      return uniqueArray(fields.map(function (item) {
  ...
  function getContractDuplicateGuards(contract) {
    try {
      var source = contract || {};
      var fields = source.duplicate_formdata_guards ||
        source.duplicateFormdataGuards ||
        [
          "object_kind",
          "family_profile_id",
          "variant_profile_id",
  ...
  function ensureDefinitionVariantHiddenFields(form) {
    try {
      ...
      return {
        ...
        vplibUid: ensureHiddenField(safeForm, FIELD_NAMES.vplibUid, "")
      };
    } catch (error) {
  ...
  function normalizeDefinitionVariant(variant, index, context) {
    try {
      ...
      var rawId = firstKeyValue(safeVariant, VARIANT_ID_KEYS);
      var rawLabel = firstKeyValue(safeVariant, VARIANT_LABEL_KEYS);
      var rawDescription = firstKeyValue(safeVariant, VARIANT_DESCRIPTION_KEYS);
  ...
  function normalizeUploadPayload(payload, fallbackKind, fallbackFieldKey) {
    try {
      ...
      var backendEnabled = source.backend_enabled !== undefined || source.backendEnabled !== undefined
        ? toBoolean(source.backend_enabled !== undefined ? source.backend_enabled : source.backendEnabled, getUploadBackendEnabled())
        : getUploadBackendEnabled();
  ...
  function ensureDefinitionVariantAliases(payload) {
    try {
      ...
      mirrorAlias(payload, "object_kind", "objectKind");
      mirrorAlias(payload, "taxonomy_path", "taxonomyPath");
  ...
  function mirrorAlias(object, snakeKey, camelKey) {
    try {
      if (object[snakeKey] !== undefined && object[snakeKey] !== null && object[snakeKey] !== "" && (object[camelKey] === undefined || object[camelKey] === null || object[camelKey] === "")) {
        object[camelKey] = object[snakeKey];
      }

      if (object[camelKey] !== undefined && object[camelKey] !== null && object[camelKey] !== "" && (object[snakeKey] === undefined || object[snakeKey] === null || object[snakeKey] === "")) {
  ...
  function syncProfileIdsIntoForm(form, options) {
    try {
      ...
      var uid = getExistingVplibUid({}, safeForm, getCreateContext(), getGeneratorContext());
      if (fields.vplibUid && uid && fields.vplibUid.value !== uid) {
        fields.vplibUid.value = uid;
      }
  ...
  function getCurrentVariantProfileBundle() {
    try {
      ...
      if (profilesRuntime && typeof profilesRuntime.getResolvedProfileBundleSync === "function") {
        var syncBundle = profilesRuntime.getResolvedProfileBundleSync();

        if (syncBundle && typeof syncBundle === "object") {
          return syncBundle;
        }
      }
      ...
      if (profilesRuntime && typeof profilesRuntime.getCurrentProfilePayload === "function") {
        return {
          payload: profilesRuntime.getCurrentProfilePayload() || {},
          context: profilesRuntime.getCurrentContext ? profilesRuntime.getCurrentContext() || {} : {}
        };
      }
  ...
  function summarizePayload(payload) {
    try {
      ...
      return {
        ...
        validation: clone(localState.lastValidation),
        timestamp: timestamp()
      };
    } catch (error) {
      return {
  ...
  function getUploadConfig() {
    try {
      var context = getCreateContext();
      return window.VectoplanCreateUploadConfig ||
  ...
  function getUploadBackendEnabled() {
    try {
      var config = getUploadConfig();

      if (config.backend_enabled !== undefined) {
        return toBoolean(config.backend_enabled, true);
      }

      if (config.backendEnabled !== undefined) {
        return toBoolean(config.backendEnabled, true);
      }
  ...
  function getDefaultValues() {
    try {
      ...
      var defaults = Object.assign(
        {},
        getNested(core || {}, ["state", "uiState", "defaults"], {}),
        context.defaults || {},
        uiState.defaults || {},
        getNested(getGeneratorContext(), ["data", "default_values"], {}),
        getNested(getGeneratorContext(), ["default_values"], {})
      );
  ...
  function getExistingVplibUid(payload, form, context, generatorContext) {
    try {
      var candidates = [
        payload.vplib_uid,
        payload.vplibUid,
        getFieldValue(form, FIELD_NAMES.vplibUid),
        context.vplib_uid,
        context.vplibUid,
        getNested(context, ["context", "vplib_uid"], ""),
        getNested(context, ["generatorContext", "vplib_uid"], ""),
        generatorContext.vplib_uid,
        generatorContext.vplibUid,
        getNested(generatorContext, ["data", "vplib_uid"], ""),
        getNested(generatorContext, ["published", "vplib_uid"], "")
      ];
  ...
  function buildTaxonomyPath(source) {
    try {
      return normalizeTaxonomyPath([
        source.domain,
        source.category,
        source.subcategory
      ].filter(Boolean).join("/"));
  ...
  function normalizeSingleValueFields(payload) {
    try {
      SCALAR_FIELD_NAMES.forEach(function (fieldName) {
        if (payload[fieldName] !== undefined) {
          payload[fieldName] = firstScalar(payload[fieldName]);
    ...
    try {
      if (Array.isArray(value)) {
        for (var index = value.length - 1; index >= 0; index -= 1) {
          if (value[index] !== null && value[index] !== undefined && String(value[index]).trim() !== "") {
  ...
  function emptyUploadPayload(kind) {
    try {
      ...
      var backendEnabled = getUploadBackendEnabled();
