/* services/vectoplan-library/static/library_admin/js/create_uploads.js */
/* -----------------------------------------------------------------------------
  VECTOPLAN Library · VPLIB Create Wizard · Upload Adapter

  Zweck:
  - Zentraler defensiver Frontend-Adapter für Upload-Felder im Create-Wizard.
  - Verarbeitet aktuell nur lokale File-Metadaten.
  ...
  Wichtiger Fix 0.6.1:
  - Verhindert die Event-Schleife:
      create_uploads.syncZone()
      → upload-changed
      → create_payload.syncUploadsRuntimeToForm()
      → create_uploads.syncAll()
      → upload-changed
      → ...
  - syncAll() läuft standardmäßig still.
  - writeMetadata() feuert standardmäßig keine nativen input/change Events.
  - upload-changed wird nur bei echter Nutzeränderung oder explizitem emitEvents
    ausgelöst.
  - Re-Entrancy-Guards verhindern parallele Zone-Syncs.
...
(function () {
  ...
  function getPayloadForZone(zone) {
    try {
      ...
      var backendEnabled = readBool(
        zone ? zone.getAttribute("data-vp-upload-backend-enabled") : "",
        false
      );
