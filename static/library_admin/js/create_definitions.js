// services/vectoplan-library/static/library_admin/js/create_definitions.js
/* -----------------------------------------------------------------------------
  VECTOPLAN Library · Create Definitions Runtime Bridge

  Zweck:
  - Kompatibilitätsbrücke zwischen der alten create_definitions.js-Runtime und
    der neuen definition-managed Variantenruntime.
  - Stellt weiterhin window.VectoplanCreateDefinitionsRuntime bereit.
  - Delegiert Variantenlogik an:
  ...
  - Arbeitet nur noch als Bridge, Diagnose- und Legacy-API-Schicht.
  - Erzeugt keinen eigenen alten Drawer mehr.
  - Rendert keine eigenen Felder mehr.
  - Fügt keine eigenen Variantenzeilen mehr direkt ein.
  - Interceptet Add/Edit-Klicks nicht mehr mit stopPropagation.
  - Synchronisiert nur noch defensiv Legacy-Hidden-Fields und Events.

  Wichtiger Fix:
  - Kein rekursiver Aufruf von VectoplanCreateVariantState.sync()
    innerhalb von variant-state-synced / variant-state-changed.
  - definition_variants_json wird nur geschrieben, wenn sich der Wert ändert.
  - Bridge-Sync feuert keine nativen input/change Events.
  - Profil-Hidden-Fields feuern keine input/change Events mehr.
  - Programmatic Events werden in Legacy-Fallbacks ignoriert.

  Architekturregel:
  - State ist Wahrheit: window.VectoplanCreateVariantState.
  - Profile Resolution: window.VectoplanCreateVariantProfiles.
  - Drawer: window.VectoplanCreateVariantDrawer.
  - Tabelle: window.VectoplanCreateVariantTable.
  - Validation: window.VectoplanCreateVariantValidation.
  - Summary: window.VectoplanCreateVariantSummary.
  - Diese Datei ist nur noch Adapter.
...
(function () {
  ...
  function resolveDefinitionsApi() {
    try {
      if (
        ...
        window.VectoplanCreateContext.definitionsApi &&
        typeof window.VectoplanCreateContext.definitionsApi === "object"
      ) {
        return window.VectoplanCreateContext.definitionsApi;
      }
  ...
  function resolveDefinitions() {
    try {
      ...
      if (
        window.VectoplanCreateContext &&
        window.VectoplanCreateContext.definitions &&
        typeof window.VectoplanCreateContext.definitions === "object"
      ) {
        return window.VectoplanCreateContext.definitions;
      }
  ...
  function resolveDefinitionRoutes(context, definitions) {
    try {
      ...
      routes.options = routes.options || DEFAULT_DEFINITIONS_PREFIX + "/options";
      routes.payload = routes.payload || DEFAULT_DEFINITIONS_PREFIX + "/payload";
      routes.resolve_family_profile = routes.resolve_family_profile || routes.resolveFamilyProfile || DEFAULT_DEFINITIONS_PREFIX + "/resolve-family-profile";
      routes.resolve_variant_profile = routes.resolve_variant_profile || routes.resolveVariantProfile || DEFAULT_DEFINITIONS_PREFIX + "/resolve-variant-profile";
      routes.variant_profile_base = routes.variant_profile_base || routes.variantProfileBase || DEFAULT_DEFINITIONS_PREFIX + "/variant-profiles/";
      routes.empty_variant_values = routes.empty_variant_values || routes.emptyVariantValues || DEFAULT_DEFINITIONS_PREFIX + "/empty-variant-values";
      routes.empty_variant_values_base = routes.empty_variant_values_base || routes.emptyValuesBase || DEFAULT_DEFINITIONS_PREFIX + "/empty-variant-values/";
      routes.validate_variant = routes.validate_variant || routes.validateVariant || DEFAULT_DEFINITIONS_PREFIX + "/validate-variant";
    ...
    try {
      ...
      document.addEventListener("vectoplan:create:variant-state-changed", function (event) {
        try {
          ...
          warn("Variant-state-changed listener failed.", error);
        }
      });
      ...
      document.addEventListener("vectoplan:create:variant-state-synced", function (event) {
        try {
          syncVariantsFromEvent(event && event.detail ? event.detail : {}, {
            source: "variant-state-synced"
          });
        } catch (error) {
          warn("Variant-state-synced listener failed.", error);
        }
      });
      ...
      document.addEventListener("vectoplan:create:variant-removed", function (event) {
        try {
          syncVariantsFromEvent(event && event.detail ? event.detail : {}, {
            source: "variant-removed"
          });
        } catch (error) {
          warn("Variant-removed listener failed.", error);
        }
      });
    ...
    try {
      ...
      document.addEventListener("change", function (event) {
        try {
          ...
          if (state.suppressProfileFieldChange) {
            return;
          }
  ...
  /* ---------------------------------------------------------------------------
   * Legacy API: Drawer / State / Validation
   * ------------------------------------------------------------------------ */
  ...
  /* ---------------------------------------------------------------------------
   * Variants / Hidden Field Compatibility
   * ------------------------------------------------------------------------ */
