// static/library_admin/js/create_variant_validation.js
/* -----------------------------------------------------------------------------
 * VECTOPLAN Library · Create Wizard · Variant Validation
 *
 * Zweck:
 * - Zentrale Validierungsschicht für definition-managed Varianten.
 * - Validiert lokal anhand:
 *     - Variant Profile `required_fields`
 *     - Variable Definitions `value_type`, `widget`, `validation`, `options`
 *     - systemverwalteter Kernfelder wie `variant.variant_id`
 * - Kann optional Backend-Validation aufrufen:
 *     - POST /api/v1/vplib/definitions/validate-variant
 * - Schreibt Validierungsfehler in den Drawer.
 * - Dispatcht maschinenlesbare Events für Drawer, Footer, Table und State.
 *
 * Architekturregel:
 * - Diese Datei entscheidet nicht, welche fachlichen Felder existieren.
 * - Sie prüft nur gegen das geladene Profil und die Variable Definitions.
 * - Sie rendert keine fachlichen Eingabefelder.
 * - Sie erzeugt keine Varianten.
 ...
 *
 * Events:
 * - dispatch: vectoplan:create:variant-validation-ready
 * - dispatch: vectoplan:create:variant-validation-started
 * - dispatch: vectoplan:create:variant-validation-finished
 * - dispatch: vectoplan:create:variant-validation-failed
 * - dispatch: vectoplan:create:variant-validation-cleared
 * - dispatch: vectoplan:create:variant-field-validation-updated
 *
 ...
 * - listen: vectoplan:create:variant-drawer-validate-requested
 * -------------------------------------------------------------------------- */
...
(function () {
  ...
  var CONTROL_SELECTOR = "[data-vp-field-input='true'], [data-vp-field-control-input='true']";
  ...
  var fallbackUtils = {
    ...
    normalizeValueForVariable: function (value, variable) {
      try {
        var type = variable && (variable.value_type || variable.type) ? String(variable.value_type || variable.type) : "";
  ...
  function getVariantProfile(profileId) {
    try {
      ...
      if (
        ...
        if (local && local.ok) {
          return local.variant_profile || local.profile || null;
        }
      }
  ...
  function getRequiredFields(profile) {
    try {
      return U().toArray(profile && profile.required_fields);
    } catch (error) {
      return [];
    }
  }
  ...
  function getProfileFields(profile) {
    try {
      ...
      U().toArray(profile && profile.sections).forEach(function (section) {
        U().toArray(section && section.fields).forEach(function (fieldKey) {
      ...
      U().toArray(profile && profile.required_fields).forEach(function (fieldKey) {
        if (fieldKey && !seen[fieldKey]) {
          seen[fieldKey] = true;
          fields.push(fieldKey);
        }
      ...
      U().toArray(profile && profile.optional_fields).forEach(function (fieldKey) {
        if (fieldKey && !seen[fieldKey]) {
          seen[fieldKey] = true;
          fields.push(fieldKey);
        }
  ...
  function isRequired(profile, fieldKey, variable) {
    try {
      ...
      if (variable && U().bool(variable.required_default, false)) {
        return true;
      }
  ...
  function buildVariantFromDrawer(root) {
    try {
      ...
      return {
        ...
        variant_profile_id: context.variant_profile_id || (profile ? profile.id : ""),
  ...
  function getOptionValues(variable) {
    try {
      return U().toArray(variable && variable.options).map(function (option) {
  ...
  function labelForField(fieldKey, variable) {
    try {
      return variable && variable.label ? variable.label : fieldKey || "Feld";
    } catch (error) {
  ...
  function validateValidationRules(fieldKey, value, variable) {
    try {
      if (!variable || !variable.validation || isEmpty(value)) {
        return [];
      }

      var issues = [];
      var rules = variable.validation || {};
      var type = variable.value_type || variable.type || "string";

      if ((type === "number" || type === "integer" || type === "money") && !isEmpty(value)) {
        var number = type === "integer" ? U().intValue(value, null) : U().floatValue(value, null);

        if (number !== null && !isNaN(number)) {
          if (rules.min !== undefined && rules.min !== null && number < Number(rules.min)) {
            issues.push(makeIssue("error", "min", fieldKey, variable, "Wert darf nicht kleiner als " + String(rules.min) + " sein.", {
              min: rules.min,
              value: number
            }));
          }

          if (rules.max !== undefined && rules.max !== null && number > Number(rules.max)) {
            issues.push(makeIssue("error", "max", fieldKey, variable, "Wert darf nicht größer als " + String(rules.max) + " sein.", {
              max: rules.max,
              value: number
            }));
          }
      ...
      if ((type === "string" || type === "text" || type === "url" || type === "date" || !type) && typeof value === "string") {
        if (rules.min_length !== undefined && rules.min_length !== null && value.length < Number(rules.min_length)) {
        ...
        if (rules.max_length !== undefined && rules.max_length !== null && value.length > Number(rules.max_length)) {
  ...
  function validateEnum(fieldKey, value, variable) {
    try {
      ...
      var type = variable.value_type || variable.type || "";
      var widget = variable.widget || "";
  ...
  function localValidateValues(values, profile, options) {
    try {
      ...
      if (!sourceProfile) {
        ...
        return buildValidationResult(false, errors, warnings, sourceValues, sourceProfile, "local");
      }
  ...
  function backendValidateVariant(variant, profile, options) {
    try {
      ...
      var source = normalizeVariantInput(variant || {});
      var profileId = source.variant_profile_id || (profile ? profile.id : "") || "";
  ...
  function validateValues(values, profile, options) {
    try {
      var variant = {
        ...
        variant_profile_id: profile ? profile.id || "" : "",
        definition_values: values || {}
      };
  ...
  function validateDrawer(input) {
    try {
      var root = input && input.root ? input.root : null;
      var cache = cacheDrawer(root);
      var variant = input && input.variant ? input.variant : buildVariantFromDrawer(cache.drawer);
      var profile = input && input.profile ? input.profile : readDrawerProfile(cache.drawer);

      return validateVariant(variant, U().safeMerge(input || {}, {
        profile: profile,
        context: readDrawerContext(cache.drawer),
        render: true,
        root: cache.drawer
      }));
    } catch (error) {
    ...
    try {
      ...
      issues.forEach(function (issue) {
        try {
          ...
          var field = U().qs("[data-vp-field-key='" + fieldKey + "']", drawer);
  ...
  function setStatusFromResult(cache, result) {
    try {
      var state = result.ok ? "valid" : "invalid";
      var message = "";
      ...
      if (cache.statusPill) {
        cache.statusPill.className = "vp-create-variant-drawer__status-pill vp-create-variant-drawer__status-pill--" + state;
    ...
    try {
      ...
      document.addEventListener("vectoplan:create:variant-drawer-opened", function (event) {
        try {
          ...
          if (payload.values || payload.definition_values) {
            runtime.lastValues = payload.values || payload.definition_values;
          } else if (payload.valuesJson || payload.definition_values_json) {
            runtime.lastValues = U().valuesFromJson(payload.valuesJson || payload.definition_values_json);
          }
