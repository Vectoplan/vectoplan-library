/* services/vectoplan-library/static/library_admin/js/create_variant_table.js */
/* -----------------------------------------------------------------------------
 * VECTOPLAN Library · Create Wizard · Variant Table
 *
 * Zweck:
 * - Zentrale Tabellen-/Listen-Runtime für definition-managed Varianten.
 * - Rendert die Variantenliste aus `window.VectoplanCreateVariantState`.
 * - Hält die bestehenden Hidden Fields für die Create-/Backend-Kompatibilität.
 * - Die Tabelle ist read-only: Bearbeitung läuft ausschließlich über den Drawer.
 * - Entfernen läuft über den zentralen Variant State.
 * - Default-Variante bleibt geschützt.
 *
 * Architekturregel:
 * - State ist Wahrheit.
 * - Tabelle ist Anzeige + Legacy-/Form-Bridge.
 * - Keine fachlichen Felder in der Tabelle.
 * - Keine Profile-Resolution.
 * - Keine Field-Rendering-Logik.
 * - Keine finale Validation.
 * - Keine VPLIB-/Package-Erzeugung.
 *
 * UI-Ziel dieser Fassung:
 * - Tabelle kennt Editor-Modus open/closed.
 * - Bei geöffnetem Variant Editor wird die Variantenübersicht ausgeblendet.
 * - Während der Editor offen ist, wird kein sichtbarer Re-Render-Sturm erzeugt.
 * - State-Sync aktualisiert weiter das Hidden JSON, aber nicht die sichtbare Liste.
 * - Nach Apply/Close/Reset wird die Übersicht wieder sichtbar und aktualisiert.
 * - syncJsonField() bleibt idempotent und feuert standardmäßig keine nativen Events.
 * - requestEdit() und requestRemove() lösen nicht doppelt Drawer/State aus.
 * - additional_field_keys werden als Hidden Field mitgeführt.
 * - Drawer-Slot bleibt im Editor-Modus sichtbar, Table-Slot wird ausgeblendet.
 *
 * Global:
 ...
 *
 * Events:
 * - dispatch: vectoplan:create:variant-table-controller-ready
 * - dispatch: vectoplan:create:variant-table-render-started
 * - dispatch: vectoplan:create:variant-table-rendered
 * - dispatch: vectoplan:create:variant-table-render-failed
 * - dispatch: vectoplan:create:variant-table-editor-mode-changed
 * - dispatch: vectoplan:create:variant-edit-requested
 * - dispatch: vectoplan:create:variant-state-remove-requested
 *
 * - listen: vectoplan:create:variant-state-ready
 ...
 * - listen: vectoplan:create:variant-editor-mode-changed
...
(function () {
  ...
  var DEFAULT_FIELD_SELECTOR = "[data-vp-default-variant-id-field='true'], [data-vp-definition-variants-default-id='true']";
  ...
  var fallbackUtils = {
    ...
    bool: function (value, fallback) {
      try {
        ...
        if (["true", "1", "yes", "ja", "on", "ok", "default", "selected"].indexOf(text) !== -1) {
          return true;
        }
  ...
  function requestEdit(row) {
    try {
      ...
      if (getDrawerApi() && typeof getDrawerApi().open === "function") {
        getDrawerApi().open(payload).catch(function (error) {
