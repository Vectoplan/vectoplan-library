/* services/vectoplan-library/static/library_admin/js/create_variant_profiles.js */
/* -----------------------------------------------------------------------------
 * VECTOPLAN Library · Create Wizard · Variant Profiles
 *
 * Zweck:
 * - Zentrale Profile-/Definitions-Schicht für den definition-managed
 *   Varianten-Flow.
 * - Kapselt Zugriff auf Backend-Definitions-APIs.
 * - Kapselt lokale Fallback-Auflösung über bereits geladene Definitionsdaten.
 * - Resolved:
 *     - Family Profile
 *     - Variant Profile
 *     - Variant Profile Payload
 *     - Empty Variant Values
 * - Schreibt resolved Profile-IDs in Workspace-, Drawer- und Hidden-Felder.
 * - Dispatcht robuste Events für State, Drawer, Table, Field Renderer und
 *   Validation.
 *
 * Architekturregel:
 * - Diese Datei entscheidet nicht fachlich neu.
 * - Sie nutzt Backend-Definitionen und lokale Katalogdaten als Spiegel.
 * - Sie rendert keine Felder.
...
(function () {
  ...
  var fallbackUtils = {
    ...
    buildDefinitionMaps: function (defs) {
      try {
        var normalized = fallbackUtils.normalizeDefinitions(defs);

        return {
          objectKindsById: fallbackUtils.indexBy(normalized.object_kinds, "id"),
          familyProfilesById: fallbackUtils.indexBy(normalized.family_profiles, "id"),
          variantProfilesById: fallbackUtils.indexBy(normalized.variant_profiles, "id"),
          variablesByKey: fallbackUtils.indexBy(normalized.variables, "key"),
          unitsById: fallbackUtils.indexBy(normalized.units, "id"),
          materialsById: fallbackUtils.indexBy(normalized.materials, "id"),
          documentTypesById: fallbackUtils.indexBy(normalized.document_types, "id"),
          profileBindingsById: fallbackUtils.indexBy(normalized.profile_bindings, "id")
        };
      } catch (error) {
        return {
          objectKindsById: {},
  ...
  function readEndpointFromContext(key, fallback) {
    try {
      ...
      var api = context.definitionsApi || context.definitions_api || {};

      if (api && api[key]) {
        return api[key];
      }

      if (api.routes && api.routes[key]) {
        return api.routes[key];
      }

      if (api.endpoints && api.endpoints[key]) {
        return api.endpoints[key];
      }
  ...
  function responseOk(payload) {
    try {
      ...
      return ["ok", "healthy", "ready", "success"].indexOf(status) !== -1;
    } catch (error) {
      return false;
    }
  }
  ...
  function readDefinitionsFromWindow() {
    try {
      ...
      for (var index = 0; index < candidates.length; index += 1) {
        var normalized = U().normalizeDefinitions(candidates[index]);

        if (
          normalized.variant_profiles.length ||
          normalized.variables.length ||
          normalized.profile_bindings.length ||
          normalized.family_profiles.length
        ) {
          return normalized;
        }
      }
  ...
  function getDefinitionsSync(options) {
    try {
      ...
      runtime.cache.definitionMaps = U().buildDefinitionMaps(runtime.cache.definitions);
  ...
  function getDefinitionMaps(options) {
    try {
      ...
      runtime.cache.definitionMaps = U().buildDefinitionMaps(getDefinitionsSync(config));
      return runtime.cache.definitionMaps;
    } catch (error) {
  ...
  function fetchDefinitions(options) {
    try {
      ...
      return requestJson(endpoints.options, {
        ...
        var definitions = U().normalizeDefinitions(data);
        ...
        runtime.cache.definitionMaps = U().buildDefinitionMaps(definitions);
    ...
    try {
      ...
      var context = {
        ...
        object_kind: firstValue(FIELD_SELECTORS.objectKind, document) ||
          U().attr(contextRoot, "data-vp-current-object-kind", "cell_block"),
      ...
      context.object_kind = U().normalizeObjectKind(context.object_kind || "cell_block");
      context.family_profile_id = U().normalizeProfileId(context.family_profile_id || "");
      context.variant_profile_id = U().normalizeProfileId(context.variant_profile_id || "");
  ...
  function normalizeContext(context) {
    try {
      ...
      return {
        domain: U().trim(raw.domain || raw.taxonomy_domain || ""),
        category: U().trim(raw.category || raw.taxonomy_category || ""),
        subcategory: U().trim(raw.subcategory || raw.taxonomy_subcategory || ""),
  ...
  function bindingMatches(binding, context, mode) {
    try {
      ...
      if (!valueMatches(item.domain || item.taxonomy_domain, ctx.domain)) {
        return false;
      }

      if (!valueMatches(item.category || item.taxonomy_category, ctx.category)) {
        return false;
      }

      if (!valueMatches(item.subcategory || item.taxonomy_subcategory, ctx.subcategory)) {
        return false;
      }
      ...
      if (mode === "variant" && ctx.family_profile_id) {
        var bindingFamily = item.family_profile_id || item.familyProfileId || "";
  ...
  function bindingScore(binding, context) {
    try {
      ...
      if ((item.domain || item.taxonomy_domain) && valueMatches(item.domain || item.taxonomy_domain, ctx.domain)) {
        score += 10;
      }

      if ((item.category || item.taxonomy_category) && valueMatches(item.category || item.taxonomy_category, ctx.category)) {
        score += 20;
      }

      if ((item.subcategory || item.taxonomy_subcategory) && valueMatches(item.subcategory || item.taxonomy_subcategory, ctx.subcategory)) {
        score += 30;
      }
      ...
      if ((item.family_profile_id || item.familyProfileId) && ctx.family_profile_id && (item.family_profile_id || item.familyProfileId) === ctx.family_profile_id) {
        score += 50;
      }
  ...
  function resolveFamilyProfileLocal(context) {
    try {
      ...
      if (matchingBindings.length) {
        ...
        var familyId = binding.family_profile_id || binding.familyProfileId || "";
  ...
  function resolveVariantProfileLocal(context) {
    try {
      ...
      var familyProfileId = ctx.family_profile_id || familyResult.family_profile_id || "";
      ...
      if (matchingBindings.length) {
        var binding = matchingBindings[0];
        var variantId = binding.variant_profile_id || binding.variantProfileId || "";
      ...
      if (familyProfileId && maps.familyProfilesById[familyProfileId]) {
        ...
        var defaultVariantProfileId = familyProfile.default_variant_profile_id || familyProfile.defaultVariantProfileId || "";
  ...
  function normalizeFamilyResult(payload, context, source) {
    try {
      ...
      var familyProfileId = data.family_profile_id ||
  ...
  function normalizeVariantResult(payload, context, source) {
    try {
      ...
      var variantProfileId = data.variant_profile_id ||
  ...
  function getEmptyValuesLocal(profileId, context) {
    try {
      var profileResult = getVariantProfileLocal(profileId);
      ...
      function addDefaultForKey(key) {
        try {
          ...
          if (Object.prototype.hasOwnProperty.call(variable, "default_value")) {
            values[key] = U().deepClone(variable.default_value, null);
            return;
          }
      ...
      if (U().getProfileFieldKeys) {
        fieldKeys = U().getProfileFieldKeys(profile);
      } else {
        U().toArray(profile.sections).forEach(function (section) {
          U().toArray(section.fields).forEach(function (fieldKey) {
      ...
      fieldKeys.forEach(addDefaultForKey);
  ...
  function updateProfileAttrs(result, options) {
    try {
      var config = options || {};
      var familyProfileId = result.family_profile_id || "";
      var variantProfileId = result.variant_profile_id || "";
      var changed = false;
  ...
  function initialize(options) {
    try {
      ...
      if (hasDefinitionData(getDefinitionsSync())) {
        ...
        if (config.fetchDefinitions !== false) {
          fetchDefinitions({
