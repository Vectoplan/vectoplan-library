/* services/vectoplan-library/static/library_admin/js/create_variant_optional_fields.js */

/* -----------------------------------------------------------------------------
  VECTOPLAN Library · Create Variant Optional Fields Runtime

  Zweck:
  - Ergänzt den Variant Drawer um einen optionalen Backend-Variablenkatalog.
  - Variablen kommen aus Backend-Definitionsdaten, nicht aus hart kodiertem UI.
  - Profilfelder bleiben Kernfelder und werden nicht doppelt angeboten.
  - Systemfelder wie variant.variant_id werden nie sichtbar angeboten.
  - Zusätzlich gewählte Variablen werden in definition_values geschrieben.
  - Die Auswahl wird über additional_field_keys stabil erhalten.
  - Bestehende Werte aus Varianten werden beim erneuten Öffnen wieder angezeigt.

  UI-Ziel dieser Fassung:
  - Keine sichtbare Suche.
  - Keine sichtbare Gruppenfilterung.
  - Kein "Suche leeren".
  - Kein "Aktualisieren".
  - Verfügbare Variablen werden als Liste angezeigt:
      Bezeichnung | Beschreibung | Art
  - Gewählte Zusatzfelder werden als Zeilen gerendert:
      Variable | Eingabefeld | Entfernen
  - Keine verschachtelten Card-in-Card-Flächen für Zusatzfelder.
  - Größere, klarere, besser lesbare Struktur.
  - Kompatibel mit neuer Clean-Shell:
      _variant_drawer_shell.html
  - Kompatibel mit Legacy-Shell:
      _variant_drawer.html

  Abhängigkeiten:
  - Muss nach create_variant_utils.js geladen werden.
  - Sollte nach create_variant_field_renderer.js geladen werden.
  - Wird von create_variant_drawer.js angesprochen.

  Erwartete optionale Globals:
  ...
  Öffentliche API:
  - window.VectoplanCreateVariantOptionalFields.initialize()
  - window.VectoplanCreateVariantOptionalFields.refresh()
  - window.VectoplanCreateVariantOptionalFields.render()
  - window.VectoplanCreateVariantOptionalFields.loadVariant(variant)
  - window.VectoplanCreateVariantOptionalFields.restoreFromVariant(variant)
  - window.VectoplanCreateVariantOptionalFields.setValues(values)
  - window.VectoplanCreateVariantOptionalFields.addField(key)
  - window.VectoplanCreateVariantOptionalFields.removeField(key)
  - window.VectoplanCreateVariantOptionalFields.collectValues()
  - window.VectoplanCreateVariantOptionalFields.syncToDrawerValues()
  - window.VectoplanCreateVariantOptionalFields.getAdditionalFieldKeys()
  - window.VectoplanCreateVariantOptionalFields.getSelectedFieldKeys()
  - window.VectoplanCreateVariantOptionalFields.setAdditionalFieldKeys(keys)
  - window.VectoplanCreateVariantOptionalFields.getAvailableVariables()
  - window.VectoplanCreateVariantOptionalFields.getRuntimeSnapshot()
----------------------------------------------------------------------------- */
...
(function () {
  ...
  function normalizeDefinitions(raw) {
    try {
      if (U() && typeof U().normalizeDefinitions === "function") {
        return U().normalizeDefinitions(raw || {});
      }
  ...
  function isSystemVariable(variable) {
    try {
      ...
      if (U() && typeof U().shouldHideVariableInDrawer === "function") {
  ...
  function getDefinitionMaps() {
    try {
      ...
      if (U() && typeof U().buildDefinitionMaps === "function") {
        return U().buildDefinitionMaps(window.VectoplanCreateDefinitions || {});
      }
  ...
  function normalizeToken(value, fallback) {
    try {
      if (U() && typeof U().normalizeId === "function") {
        return U().normalizeId(value || fallback || "");
      }
