// services/vectoplan-library/static/library_admin/js/create_variant_drawer.js
/* -----------------------------------------------------------------------------
 * VECTOPLAN Library · Create Wizard · Variant Drawer
 *
 * Zweck:
 * - Zentrale Orchestrierung für den definition-managed Variant Drawer.
 * - Verbindet Drawer-Shell, Profile-Resolution, Field Renderer, State,
 *   Optional Fields und Validation.
 * - Öffnet den Editor für:
 *     - Variante hinzufügen
 *     - Variante bearbeiten
 *     - Default-Variante bearbeiten
 * - Lädt/merged Empty Values, vorhandene Variant Values und Profil-Defaults.
 * - Sammelt Drawer-Werte inklusive optional gewählter Backend-Variablen.
 * - Vergibt systemverwaltete Variant-IDs.
 * - Schreibt Varianten über window.VectoplanCreateVariantState.
 *
 * Architekturregel:
 * - Diese Datei rendert keine fachlichen Felder selbst.
 * - Diese Datei entscheidet nicht, welche fachlichen Felder existieren.
 * - Diese Datei ruft Profile-/FieldRenderer-/Validation-/State-Schichten auf.
 * - `variant.variant_id` ist systemverwaltet.
 * - Nutzer pflegt `variant.label` und fachliche Werte.
 *
 * UI-Ziel dieser Fassung:
 * - Clean-Shell wird als eingebetteter Full-Width-Editor behandelt.
 * - Variantenübersicht wird beim Bearbeiten ausgeblendet.
 * - Step-3-Objektartbereich wird beim Bearbeiten ausgeblendet.
 * - Editor-State wird auf Workspace, Drawer, Table-Slot und Step-3-Section
 *   gespiegelt.
 * - Linke Section-Navigation wird im Clean-Modus nicht benötigt.
 * - Im Clean-Modus werden Sections nicht mehr gegenseitig ausgeblendet.
 * - Falls Legacy-Drawer genutzt wird, bleibt Section-Navigation funktionsfähig.
 * - Debug-/Meta-/Statusflächen bleiben intern nutzbar, aber die Clean-Shell
 *   zeigt sie nicht sichtbar an.
 * - Optional Fields werden aus additional_field_keys wiederhergestellt.
 * - additional_field_keys werden beim Speichern in die Variante geschrieben.
 * - Der Apply-Button heißt fachlich "Variante speichern".
 *
 * Global:
 ...
 *
 * Events:
 * - dispatch: vectoplan:create:variant-drawer-ready
 * - dispatch: vectoplan:create:variant-editor-mode-changed
 * - dispatch: vectoplan:create:variant-drawer-session-started
 * - dispatch: vectoplan:create:variant-drawer-session-prepared
 * - dispatch: vectoplan:create:variant-drawer-session-failed
 * - dispatch: vectoplan:create:variant-drawer-section-changed
 * - dispatch: vectoplan:create:variant-drawer-validate-started
 * - dispatch: vectoplan:create:variant-drawer-validate-finished
 * - dispatch: vectoplan:create:variant-drawer-apply-started
 * - dispatch: vectoplan:create:variant-drawer-apply-failed
 * - dispatch: vectoplan:create:variant-drawer-apply-finished
 * - dispatch: vectoplan:create:variant-drawer-reset
 *
 * - listen: click on add/edit buttons as fallback
 * - listen: click on section navigation buttons
 * - listen: vectoplan:create:variant-add-requested
 * - listen: vectoplan:create:variant-edit-requested
 * - listen: vectoplan:create:variant-drawer-apply-requested
 * - listen: vectoplan:create:variant-drawer-validate-requested
 * - listen: vectoplan:create:variant-drawer-cancel-requested
 * - listen: vectoplan:create:variant-empty-state-close-requested
  ...
  function prepareSession(session, options) {
    try {
      ...
      if (profilesApi() && typeof profilesApi().getResolvedProfileBundle === "function") {
        return profilesApi().getResolvedProfileBundle(context, {
          force: !!config.force,
  ...
  function applyBundleToSession(session, bundle, options) {
    try {
      ...
      var prepared = U().safeMerge(session, {
        ...
        definition_values: values,
        definition_values_json: U().valuesToJson(values),
        additional_field_keys: normalizeAdditionalFieldKeys(session.additional_field_keys || session.additionalFieldKeys || []),
        additionalFieldKeys: normalizeAdditionalFieldKeys(session.additional_field_keys || session.additionalFieldKeys || []),
        bundle: resolved,
